...
stackByTable("/NEON/NEON_count-ticks.zip")
...
out <- runMCMC(bayesianSetup = bs, sampler = "DEzs", settings = set)

saveRDS(out, file = "~/model_estimates.rds")


#  run diagnostics
# plot(out)
# tracePlot(out)
# correlationPlot(out, whichParameters = c("log_dln_SCBI","a_n_SCBI","log_al_SCBI","log_aa_SCBI"))
# samp <- getSample(out, start = 1000)            # burn-in via 'start'
# coda_obj <- getSample(out, start = 1000, coda = TRUE)
# coda::gelman.diag(coda_obj)                     # R-hat
# summary(out); DIC(out); WAIC(out)               # model fit criteria
