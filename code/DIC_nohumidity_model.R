...
# --- FIXED NULL PRIOR SAMPLER ---
rprior_null <- function(n = 1){
  out <- rprior(n) # Generate a valid draw first
  
  # 1. Identify parameters that should be locked to Null
  # We exclude "sigma" from this list
  an_to_null <- c("a_n_bar", "a_n_BLAN", "a_n_HARV", "a_n_SCBI")
  la_to_null <- c("log_al_bar", "log_al_BLAN", "log_al_HARV", "log_al_SCBI",
                  "log_aa_bar", "log_aa_BLAN", "log_aa_HARV", "log_aa_SCBI")
  
  # 2. Force ONLY these to the Null values
  out[, an_to_null] <- 22.0
  out[, la_to_null] <- -10.0
  
  # Note: a_n_sigma and log_a_sigma stay as positive values from rprior()
  return(out)
}

# --- FIXED NULL PRIOR DENSITY ---
dprior_null <- function(par, log = TRUE){
  names(par) <- param_names
  
  # Check only the non-sigma humidity parameters
  check_an <- c("a_n_bar", "a_n_BLAN", "a_n_HARV", "a_n_SCBI")
  check_la <- c("log_al_bar", "log_al_BLAN", "log_al_HARV", "log_al_SCBI",
                "log_aa_bar", "log_aa_BLAN", "log_aa_HARV", "log_aa_SCBI")
  
  is_null_an  <- all(abs(par[check_an] - 22) < 1e-2)
  is_null_log <- all(abs(par[check_la] + 10) < 1e-2)
  
  if(!is_null_an || !is_null_log) return(-Inf)
  
  # Calculate priors for EVERYTHING ELSE (dln, alpha0, AND sigmas)
  ll <- 0
  
  # log_dln priors
  ll <- ll + dnorm(par["log_dln_bar"], m_log_dln_bar, s_log_dln_bar, log=TRUE)
  ll <- ll + dexp(par["log_dln_sigma"], rate = rate_log_dln_sigma, log=TRUE)
  ll <- ll + dnorm(par["log_dln_BLAN"], par["log_dln_bar"], par["log_dln_sigma"], log=TRUE)
  ll <- ll + dnorm(par["log_dln_HARV"], par["log_dln_bar"], par["log_dln_sigma"], log=TRUE)
  ll <- ll + dnorm(par["log_dln_SCBI"], par["log_dln_bar"], par["log_dln_sigma"], log=TRUE)
  
  # Even though they are "ghost" parameters now, the sampler needs their 
  # hyper-priors to be valid numbers.
  ll <- ll + dexp(par["a_n_sigma"], rate = rate_a_n_sigma, log=TRUE)
  ll <- ll + dexp(par["log_al_sigma"], rate = rate_log_al_sigma, log=TRUE)
  ll <- ll + dexp(par["log_aa_sigma"], rate = rate_log_aa_sigma, log=TRUE)
  
  # Nuisance log_alpha0 priors
  nuisance_pars <- names(par)[grep("log_alpha0", names(par))]
  for(p in nuisance_pars) ll <- ll + dnorm(par[p], m_log_alpha0, s_log_alpha0, log=TRUE)
...
prior_null <- BayesianTools::createPrior(density = dprior_null, 
                                         sampler = rprior_null,
                                         lower = lower, upper = upper)

# Setup the Null Model
bs_null <- createBayesianSetup(likelihood = fn, prior = prior_null, names = param_names)

# Run MCMC (Use your existing 'set' list)

set <- list(iterations = 9000, thin = 1, burnin = 1000, nrChains = 3)      

# Make sure your start value is 'Null-compatible'
start_val_null <- rprior_null(3) 
set$startValue <- start_val_null
out_null <- runMCMC(bayesianSetup = bs_null, settings = set)
saveRDS(out_null, file = "~/nohum_smoeyrs_10k.rds")
# Extract the chains, including posterior, likelihood, and prior values
samp <- getSample(out_null, parametersOnly = FALSE)

# Check for NAs or NaNs in the log-likelihood column
sum(is.na(samp[, "Llikelihood"]))

# Check for Infinities (-Inf) in the log-likelihood column
sum(is.infinite(samp[, "Llikelihood"]))

# 1. Extract just the log-likelihood column and remove the 3 NAs
clean_ll <- na.omit(samp[, "Llikelihood"])

# 2. Calculate Dbar (mean deviance of the clean samples)
Dbar <- mean(-2 * clean_ll)

# 3. Input calculated Dhat 
Dhat <- 2608.965

# 4. Calculate pD (Effective number of parameters)
pD <- Dbar - Dhat

# 5. Calculate final DIC
final_DIC <- Dbar + pD

# Print your rescued metrics!
cat("Dbar:", Dbar, "\n")
cat("Dhat:", Dhat, "\n")
cat("pD:  ", pD, "\n")
cat("DIC: ", final_DIC, "\n")
