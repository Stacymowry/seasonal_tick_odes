...
# 1. Create a "Template" vector of 46 zeros with the correct names
test_pars <- c(
  log_dln_BLAN = full_pars$log_dln_BLAN,
  a_n_BLAN     = full_pars$a_n_BLAN,
  log_al_BLAN  = full_pars$log_al_BLAN,
  log_aa_BLAN  = full_pars$log_aa_BLAN,
  
  log_alpha0_L_BLAN_2016 = median(someyrs_10k_pars[,"log_alpha0_L_BLAN_2016"]),
  log_alpha0_N_BLAN_2016 = median(someyrs_10k_pars[,"log_alpha0_N_BLAN_2016"]),
  log_alpha0_L_BLAN_2017 = median(someyrs_10k_pars[,"log_alpha0_L_BLAN_2017"]),
  log_alpha0_N_BLAN_2017 = median(someyrs_10k_pars[,"log_alpha0_N_BLAN_2017"]),
  log_alpha0_L_BLAN_2018 = median(someyrs_10k_pars[,"log_alpha0_L_BLAN_2018"]),
  log_alpha0_N_BLAN_2018 = median(someyrs_10k_pars[,"log_alpha0_N_BLAN_2018"]),
  log_alpha0_L_BLAN_2019 = median(someyrs_10k_pars[,"log_alpha0_L_BLAN_2019"]),
  log_alpha0_N_BLAN_2019 = median(someyrs_10k_pars[,"log_alpha0_N_BLAN_2019"]),
  
  log_dln_HARV = full_pars$log_dln_HARV,
  a_n_HARV     = full_pars$a_n_HARV,
  log_al_HARV  = full_pars$log_al_HARV,
  log_aa_HARV  = full_pars$log_aa_HARV,
  
  log_alpha0_L_HARV_2017 = median(someyrs_10k_pars[,"log_alpha0_L_HARV_2017"]),
  log_alpha0_N_HARV_2017 = median(someyrs_10k_pars[,"log_alpha0_N_HARV_2017"]),
  log_alpha0_L_HARV_2018 = median(someyrs_10k_pars[,"log_alpha0_L_HARV_2018"]),
  log_alpha0_N_HARV_2018 = median(someyrs_10k_pars[,"log_alpha0_N_HARV_2018"]),
  log_alpha0_L_HARV_2021 = median(someyrs_10k_pars[,"log_alpha0_L_HARV_2021"]),
  log_alpha0_N_HARV_2021 = median(someyrs_10k_pars[,"log_alpha0_N_HARV_2021"]),
  log_alpha0_L_HARV_2023 = median(someyrs_10k_pars[,"log_alpha0_L_HARV_2023"]),
  log_alpha0_N_HARV_2023 = median(someyrs_10k_pars[,"log_alpha0_N_HARV_2023"]),
  
  log_dln_SCBI = full_pars$log_dln_SCBI,
  a_n_SCBI     = full_pars$a_n_SCBI,
  log_al_SCBI  = full_pars$log_al_SCBI,
  log_aa_SCBI  = full_pars$log_aa_SCBI,
  
  log_alpha0_L_SCBI_2016 = median(someyrs_10k_pars[,"log_alpha0_L_SCBI_2016"]),
  log_alpha0_N_SCBI_2016 = median(someyrs_10k_pars[,"log_alpha0_N_SCBI_2016"]),
  log_alpha0_L_SCBI_2017 = median(someyrs_10k_pars[,"log_alpha0_L_SCBI_2017"]),
  log_alpha0_N_SCBI_2017 = median(someyrs_10k_pars[,"log_alpha0_N_SCBI_2017"]),
  log_alpha0_L_SCBI_2018 = median(someyrs_10k_pars[,"log_alpha0_L_SCBI_2018"]),
  log_alpha0_N_SCBI_2018 = median(someyrs_10k_pars[,"log_alpha0_N_SCBI_2018"]),
  log_alpha0_L_SCBI_2019 = median(someyrs_10k_pars[,"log_alpha0_L_SCBI_2019"]),
  log_alpha0_N_SCBI_2019 = median(someyrs_10k_pars[,"log_alpha0_N_SCBI_2019"]),
  log_alpha0_L_SCBI_2023 = median(someyrs_10k_pars[,"log_alpha0_L_SCBI_2023"]),
  log_alpha0_N_SCBI_2023 = median(someyrs_10k_pars[,"log_alpha0_N_SCBI_2023"])
)

full_test_vector <- setNames(numeric(length(param_names)), param_names)

# 2. Inject your 38 "Real" parameters into the template
full_test_vector[names(test_pars)] <- test_pars

# 3. (Optional) Set the remaining 8 to a generic value like 1
full_test_vector[full_test_vector == 0] <- 1 

# 4. Run the likelihood
log_lik <- fn(full_test_vector)
print(log_lik)

# --- Step 1: Define the Null Values ---

num_steps <- 10
sites <- c("BLAN", "HARV", "SCBI")
nuis_levels <- c(-1, 0, 1) # SD shifts

# ADDED log_aa HERE (Assuming -10 is the biological null for this as well)
nulls <- c(a_n = 22, log_al = -10, log_dln = -10, log_aa = -10) 
prior_sd <- 1

nuisance_names <- names(full_test_vector)[grepl("log_alpha0", names(full_test_vector))]
test_scenarios <- list("FULL MODEL (Reference)" = full_test_vector)

# --- 2. Build the Enveloped Profiles ---
for(s in sites) {
  # Added log_aa to the focal parameters being iterated over
  focal_pars <- c(paste0("a_n_", s), paste0("log_al_", s), paste0("log_dln_", s), paste0("log_aa_", s))
  
  for(par_name in focal_pars) {
    type <- gsub(paste0("_", s), "", par_name)
    null_val <- nulls[type]
    best_val <- full_test_vector[par_name]
    
    # Define the range (Null to 1.5x MAP) for all parameters uniformly
    upper_val <- null_val + (best_val - null_val) * 1.5
    val_sequence <- seq(null_val, upper_val, length.out = num_steps)
    
    # For every step in the mechanism, test 3 levels of noise
    for(k in 1:num_steps) {
      for(n_shift in nuis_levels) {
        
        bucket_val <- val_sequence[k]
        scenario_name <- sprintf("%s: %s Step %d | Nuis %d SD", s, type, k, n_shift)
        
        test_scenarios[[scenario_name]] <- {
          p <- full_test_vector
          # Set the mechanism
          p[par_name] <- bucket_val
          # Set the noise envelope
          p[nuisance_names] <- p[nuisance_names] + (n_shift * prior_sd)
          p
        }
      }
    }
...
# --- 3. Add the "Noisy Null" ---
test_scenarios[["GLOBAL NOISY NULL (+1 SD)"]] <- {
  p <- full_test_vector
  # log_aa_ is already handled in this regex, so we keep it as is
  mech_pars <- names(p)[grepl("a_n_|log_aa_|log_al_|log_dln_", names(p))]
  for(m in mech_pars) {
    if(grepl("a_n", m)) p[m] <- 22 else p[m] <- -10 # Applies -10 to log_al, log_aa, and log_dln
  }
  p[nuisance_names] <- p[nuisance_names] + (1 * prior_sd)
  p
}

# --- 4. Execution ---
results <- data.frame(Scenario = names(test_scenarios), LogLikelihood = NA)
for(i in 1:length(test_scenarios)) {
  cat(sprintf("[%d/%d] Evaluating %s...\n", i, length(test_scenarios), names(test_scenarios)[i]))
  results$LogLikelihood[i] <- fn(test_scenarios[[i]])
}

# --- 5. Summary Column ---
results$LL_Drop <- results$LogLikelihood[1] - results$LogLikelihood
print(results)
...
# Extract the actual parameter values from the test_scenarios list to plot on the x-axis
# (Since the Scenario string only has the "Step" number, we need the actual numeric values)
scenario_values <- sapply(names(test_scenarios), function(scen_name) {
  if (grepl("FULL MODEL|GLOBAL", scen_name)) return(NA)
  
  # Parse the site and parameter type out of the scenario name
  s <- str_extract(scen_name, "^[A-Z]+")
  p_type <- str_extract(scen_name, "(?<=: ).*(?= Step)")
  par_name <- paste0(p_type, "_", s)
  
  # Return the actual numeric value of that parameter for this specific run
  return(test_scenarios[[scen_name]][par_name])
})

# Build plot_data
plot_data <- results %>%
  filter(!grepl("FULL MODEL|GLOBAL", Scenario)) %>%
  mutate(
    Site = str_extract(Scenario, "^[A-Z]+"),
    Param = str_extract(Scenario, "(?<=: ).*(?= Step)"),
    Noise = paste0("Nuis ", str_extract(Scenario, "[-0-9]+ SD")),
    ActualValue = scenario_values[!is.na(scenario_values)]
  ) %>%
  # Clean up factor levels so the plot orders them nicely
  mutate(
    Site = factor(Site, levels = c("BLAN", "HARV", "SCBI")),
    # Added log_aa to factor levels
    Param = factor(Param, levels = c("a_n", "log_aa", "log_al", "log_dln")),
    Noise = factor(Noise, levels = c("Nuis -1 SD", "Nuis 0 SD", "Nuis 1 SD"))
  )

# Build map_lookup for the vertical dotted lines
map_lookup <- data.frame(
  # Changed 'each' to 4 to account for the 4 parameters per site
  Site = rep(c("BLAN", "HARV", "SCBI"), each = 4),
  # Added log_aa to the repeating list
  Param = rep(c("a_n", "log_aa", "log_al", "log_dln"), times = 3)
) %>%
  rowwise() %>%
  mutate(
    par_name = paste0(Param, "_", Site),
    BestVal = full_test_vector[par_name]
  ) %>%
  ungroup() %>%
  mutate(
    Site = factor(Site, levels = c("BLAN", "HARV", "SCBI")),
    # Added log_aa to factor levels here as well
    Param = factor(Param, levels = c("a_n", "log_aa", "log_al", "log_dln"))
  )



ggplot(plot_data, aes(x = ActualValue, y = LogLikelihood)) +
  # Reference line for the Full Model Likelihood
  geom_hline(yintercept = results$LogLikelihood[1], 
             linetype = "dashed", color = "black", alpha = 0.4) +
  # Vertical lines for the original MAP estimates for each site/param
  geom_vline(data = map_lookup, aes(xintercept = BestVal), 
             linetype = "dotted", color = "gray40") +
  # The Profile Curves, colored by Noise level
  geom_line(aes(color = Noise), linewidth = 1, na.rm = TRUE) +
  geom_point(aes(color = Noise), size = 1.5, alpha = 0.7, na.rm = TRUE) +
  # Facet: Independent x-axes for params, independent y-axes for clarity
  facet_grid(Param ~ Site, scales = "free") + 
  theme_bw() +
  scale_color_manual(values = c("Nuis -1 SD" = "#888888", # Neutral Grey
                                "Nuis 0 SD"  = "#000000", # Strong Black
                                "Nuis 1 SD"  = "#D55E00")) + # Vermillion for high noise
  theme(
  ...
  labs(
    title = "Likelihood Profiles: Mechanism Sensitivity vs. Noise Levels",
    subtitle = "Dashed = Global Reference Likelihood | Dotted = Site-Specific MAP",
    x = "Parameter Value",
    y = "Log-Likelihood"
  )
